...
<script>
// UploadExcel 思路整理
// 1.利用input type="file" 文件上传输入框 实现上传文件窗口弹出
// 2.利用input 的change事件 拿到上传的文件，然后再对文件进行上传
// 3、出于上传文件之前，可能需要对文件校验的场景（beforeUpload字段）
...
// 5.出于上传文件之前可能需要对文件校验的场景提供beforeUpload字段，可以使用者自由定
// 义校验规则。
// 3.利用XLSX插件读取×15×文件里的数据，
// 拿到数据，对数据进行整合
// 4.把整合的数据抛出去
// 5,这个组件给父组件提供了两个参数beforeUpload上传前的校验onSuccess上传成功拿到数
// 据之后可以通过这个回调把数据给父组件
